...
# === BTC & ETH Prices ===
with st.expander(t('crypto_prices'), expanded=True):
...
with st.expander(t('stablecoin_section'), expanded=True):
...
with st.expander("Tokenized RWA", expanded=True):
    if treasury_data:
...
st.markdown("---")
with st.expander(t('market_depth'), expanded=True):
